...
\section{Questions}
\begin{enumerate}
    \item Primary Question: How accurate are computational thermochemical tools in predicting experimentally measured metrics across AP-based formulations containing various catalytic metal additives?
    \item Secondary Question 1: Do predictive errors correlate with specific catalyst types, concentrations, or other formulation discrepancies?
    \item Secondary Question 2: Is atomic-scale molecular dynamics simulations (LAMMPS w/ ReaxFF) detailed enough to reveal mechanisms that can explain the variance between equilibrium-based thermochemical predictions and empirical observation within catalyzed systems?
\end{enumerate}
...
\begin{itemize}
    ...
    \item With ReaxFF, the fundamental challenge in implementation is the fidelity gap between computationally intensive quantum mechanincal (QM) approaches and the low-classical simulations \cite{Senftle2016}.
    \begin{itemize}
        \item Energy Formulation - Total energy of system ($E_{system}$) is the sum of distinct partial energy terms \cite{Han2015}. Contributions include but are not limited to bond energy, penalties for overcoordinated atoms, inclusions from undercoordinated atoms, valence angle energy, conjugation effects, torsional energy, van der Waals interactions, and Coulombic interactions. \cite{Han2015}
        \item Parameterization Scope: Original ReaxFF parameters developed for organic materials but now nonmetallic and metallic elements \cite{Han2015} \cite{Senftle2016}. Cover propellant systems including $Si/O$, $Al/Al2O3$, $Mg/MgH$, $Li$, $Fe/O/H$, and $AN$.
        \item Instability - Original architecture had discontinuity in total bond order realtive to interatomic distance, due to cutoff functions \cite{Furman2019} \cite{Leven2021}. Resulted in numerical Stability problems $\rightarrow$ poor energy conservation \& high total energy drift rates during MD sims. 
        \item Modeling Issues - Traditional distribution schemes (e.g. Charge Equilibrium (QEq) or Electronegativity Equalization Method (EEM)) lead to unrealistic oscillations of long-range charge transfer \cite{Leven2021} \cite{Han2015}. Atom-condensed Kohn-Sham DFTs to the ACKS2 scheme proposed to solve this issue \cite{Furman2019}.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Complex catalysts like Fe-based MOFs and $\text{CuCr}_2\text{O}_4$ drastically accelerate decomposition kinetics, reducing the HTD temperature by over $100^\circ C$ and substantially increasing heat release ($\Delta H$) \cite{Guo2025} \cite{Hosseini2019} \cite{Peng2025}.
    \end{itemize}
    ...
    \begin{enumerate}
        ...
        \item Nanosized Advantage: Nanosized additives (Cu, Ni, Zn) are documented to have superior catalytic activity compared to microsized counterparts due to higher surface area, accelerating exothermic decomposition \cite{Song2008} \cite{Chaturvedi2015} \cite{Yadav2021}.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Kinetics Quantify Catalytic Errors: Isoconversional methods (e.g., FWO, Kissinger, Ozawa) provide quantifiable kinetic parameters ($E_\alpha$)  that show the magnitude of the catalyst induced shift \cite{Boukeciat2025} \cite{Guo2024} \cite{Peng2025} \cite{Chandrababu2023}.
        \item Example: In copper-catalyzed systems, the activation energy $(E_\alpha)$ for AP decomposition can be reduced significantly (e.g., from 207 $kJ\cdot mol^{-1}$ to 128 $kJ\cdot mol^{-1}$) due to the specific catalyst \cite{Peng2025} \cite{Chandrababu2023}.
    \end{itemize}
...
\begin{enumerate}
    ...
    \item Kinetic Alterations: These catalysts change the thermal decomposition profile of AP by shifting the high-temperature decomposition (HTD) peak to a much lower temperature and significantly reducing the required activation energy \cite{Yadav2021} \cite{Benhammada2020} \cite{Peng2025} \cite{Li2022}.
    \item Kinetic Alterations: These catalysts change the thermal decomposition profile of AP by shifting the high-temperature decomposition (HTD) peak to a much lower temperature and significantly reducing the required activation energy \cite{Yadav2021} \cite{Benhammada2020} \cite{Peng2025} \cite{Li2022}.
...
\begin{enumerate}
    ...
    \item ReaxFF uses bond-order formalism to simulate reactivity in ways other FF can't \cite{Senftle2016}. 
    \item Bond Order - Designed to bridge computation gap in QM and classical MD due to reliance on large-scale dynamic evolution and potentials that rely on fixed connectivity. Capacity stems from calculating bond order from interatomic distances, allows for the modeling of bond formation and breaking on a differentiable Potential Energy Surface (PES) \cite{Han2015}. Needed for the long-distance covalent interactions to represent transition state structures and predict reaction barriers. 
    \item Explicit Electrons (eReaxFF) - Systems where charge and polarization dynamics are central, in redox chem or batteries, eReaxFF extension used to explicitly model valence electrons \cite{Leven2021}. Enables more accurate charge distribution and transfer modeling, critical for catalysis involving electron transfer mechanisms. Allows eReaxFF to handle charge transfer processes effectively, a limitation faced by standard ReaxFF methods with fixed charge schemes.
    \item Development branches (combustion, aqueous, batteries) show versatility \cite{Senftle2016}. Combustion branch relevant for propellant systems, has been used to model energetic materials and metal oxidation processes \cite{Chu2023} \cite{Yadav2021}. For example, the high-energy material description relevant to AP decomposition was successfully combined with the combustion branch (Senftle, 2016).
    \item 
\end{enumerate}
...
\begin{enumerate}
    ...
    \item $\text{Cu}_3(\text{BTC})_2$ reduced the AP high-temperature decomposition (HTD) peak temperature from $442^\circ C$ to around $340^\circ C$ and lowered the apparent $E_\alpha$ from 207 $kJ\cdot mol^{-1}$ to approximately 128 $kJ\cdot mol^{-1}$ \cite{Peng2025}.
...
\begin{itemize}
    ...
    \item Ability of ReaxFF to accurately model reaction dynamics \cite{Han2015} supported by improvements in numerical stability \& energy drift due to tapered force fields. Consistently developed/validated across reaction pathways, including but not limited to combustion \cite{Senftle2016}. 
    \item Supports hybrid models (eReaxFF/ReaxFF/AMBER), simulate localized reactive processes using ReaxFF while treating larger environment with conventional Molecular Mechanics (MM) force fields \cite{Leven2021}. Useful for catalysis where active site reactions are critical but larger environment effects (e.g., polymer binder) also matter.
\end{itemize}
