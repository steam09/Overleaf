...
\textbf{November 2025 - Literature Review \& Data Collection}
\begin{itemize}
    \item Literature search across databases for empirical data (Google Scholar, NASA Technical Reports, JSTOR)
    \item Extract performance from publications meeting inclusion criteria
    \item Compile base dataset of 20-25+ AP-based formulations
    \item Install and test all computational tools with sample criteria (pure AP)(rocketCEA, PROPEP, Cantera, RPA Lite)
\end{itemize}

\textbf{December 2025 - Computational Predictions}
\begin{itemize}
    \item Develop (in progress from Nov.) automation scripts for input files
    \item Test predictions across the mentioned four tools for predictive benchmark dataset
    \item Validate outputs for clarity; perform quality checks
    \item Calculate base accuracy metrics (R², RMSE, MAPE) for each tool
    \item Begin LAMMPS setup and ReaxFF parameter validation
\end{itemize}

\textbf{January 2026 - Molecular Dynamics Simulations}
\begin{itemize}
    \item Build models for 3-5 key  formulations (uncatalyzed baseline + catalyzed systems)
    \item Run LAMMPS with heating protocols (298K → 600K); 2-5 nanoseconds per system
    \item Extract kinetic parameters (activation energy, pre-exponential factors) from trajectories
    \item Identify reaction pathway discrepencies between catalyzed/uncatalyzed systems
\end{itemize}

\textbf{February 2026 - Statistical Analysis \& Model Development}
\begin{itemize}
    \item Combine LAMMPS kinetic data with formulation differences (pathways)
    \item Train Random Forest regression model in predicting computational tool error per compound type
    \item Perform feature importance analysis and model validation (cross-validation)
    \item Make the Computational Reliability Index (CRI) with confidence thresholds for trustworthiness
    \item Test CRI on withheld formulations
\end{itemize}

\textbf{March 2026 - Framework Finalization \& Paper Writing}
\begin{itemize}
    \item Make visualizations (scatter plots, heat maps, error distributions)
    \item Complete statistical significance tests (ANOVA, post-hoc comparisons)
    \item Draft Results / Discussion sections
    \item Consult advisors for clarity in discussion
    \item Create decision framework documentation for other researchers
    \item Finalize Methodology and Abstract
\end{itemize}

\textbf{April 2026 - Revision \& Presentation Preparation}
\begin{itemize}
    \item Peer review with advisor (again); incorporate feedback
    \item Finalize paper and any supplementary materials (pure code/pseudo-code, data from softwares/LAMMPS, etc.)
    \item Prepare presentation and poster
    \item Submit final paper
\end{itemize}

\textbf{May 2026 - Defense \& Dissemination}
\begin{itemize}
    \item Present research findings to board
    \item Literature defense
    \item Archive code and data in public repository (GitHub)
\end{itemize}

\subsection*{Contingency Plans}
\begin{itemize}
    \item LAMMPS simulations exceed time/resource constraints: Reduce to 3 systems with shorter production runs (1-2 ns) or number of atoms
    \item Literature data collection: Lower target to 18-20 formulations while maintaining diversity
    \item Random Forest implementation proves too much: Simplify to multivariate linear regression, consult with advisors
    \item Buffer time built into March-April for unexpected delays
\end{itemize}
